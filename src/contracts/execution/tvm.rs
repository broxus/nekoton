...
pub fn call(
    ...
    ctrls
        ...
        .map_err(|err| Error::msg(err.to_string()).context("can not put data to registers"))?;
    ...
    let code = state
        .code
        .clone()
        .ok_or(Error::msg("Account has no code"))?;
    ...
    match result {
        Err(err) => {
            let exception =
                ton_vm::error::tvm_exception(err).map_err(|err| Error::msg(err.to_string()))?;
        ...
        Ok(_) => {
            match engine.get_committed_state().get_root() {
                ...
                _ => {
                    bail!("invalid committed state".to_string())
                }
            };
            Ok((engine, account))
        }
...
pub fn call_msg(
    ...
    msg: &Message,
) -> Result<(Vec<Message>, AccountStuff)> {
    let msg_cell = msg
        .write_to_new_cell()
        .map_err(|err| anyhow::anyhow!("can not serialize message: {}", err))?;
    ...
    let function_selector = match msg.header() {
        ...
        CommonMsgInfo::ExtOutMsgInfo(_) => bail!("invalid message type"),
    };
    ...
    let actions_cell = engine
        .get_actions()
        .as_cell()
        .map_err(|e| Error::msg(e).context("Can't get actions"))?
        .clone();
    let mut actions = OutActions::construct_from_cell(actions_cell).map_err(|e| Error::msg(e))?;
